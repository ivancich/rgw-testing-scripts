...
count=10
quiet="-q"
multipart="--multipart-chunk-size-mb=5"

midout="midout-$(date +%H%M%S).out"
...
source=$(find /usr -size +22M 2>/dev/null | head -1)

echo "Building up..."
dd if=$source of=$object bs=1M count=21 2>/dev/null
...
for i in $(seq $count) ; do
    ln $object ${object}.${i}
    # (sleep 5 ; bucket_stats >> $midout) &
if true ;then
    s3cmd $quiet put --host=$host $multipart ${object}.${i} s3://${bucket} 2>>$errlog
else
    s/incomplete_multipart.py $bucket ${object}.${i} ${object}.${i} 
fi
    rm -f ${object}.${i}
done
rm -f $object
